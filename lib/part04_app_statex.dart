...
class AppStateX<T extends StatefulWidget> extends StateX<T>
    with ControllersInAppByIdMixin {
  ///
  AppStateX({
    super.controller,
  ...
  /// Called after the built-in FutureBuilder has successfully been processed.
  @override
  ...
  Future<bool> didPopRoute() async {
    ...
    for (final state in list) {
      // try {
      if (await (state as StateX).didPopRoute()) {
        pop = true;
      }
    }
    return pop;
  ...
  Future<bool> didPushRouteInformation(
    ...
    for (final state in list) {
      // try {
      if (await (state as StateX).didPushRouteInformation(routeInformation)) {
        handled = true;
      }
    }
    return handled;
  ...
  void _onError(FlutterErrorDetails details) {
    ...
    final stateCalled = onStateError(details);
  ...
  bool onStateError(FlutterErrorDetails details) {
    //
    final state = lastState as StateX?;

    // If it's this object, it will call its own later
    bool stateCalled = state != null && state != this;
