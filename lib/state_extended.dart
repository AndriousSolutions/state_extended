...
/// A unified StatefulWidget with a StateX State object.
class Swx extends StatefulWidget {
  ///
  Swx(
    this.builder, {
    Key? key,
    ...
    this.inactiveLifecycleState,
    this.pausedLifecycleState,
    this.detachedLifecycleState,
    this.resumedLifecycleState,
    this.didHaveMemoryPressure,
    this.didChangeAccessibilityFeatures,
    this.onError,
  })  : _con = controller ?? StateXController(),
        super(key: key);
  ...
  /// The Controller used.
  final StateXController _con;

  /// Function to return from the build() function,
  final Widget Function(BuildContext context) builder;

  // /// Provide the setState() function to this StatefulWidget
  // void setState(VoidCallback fn) => _con.state?.setState(fn);
  //
  // /// It's BuildContext
  // BuildContext get context => _con.state!.context;

  @override
  _StateXWidgetState createState() => _StateXWidgetState();

  ///
  final Future<bool> Function()? initAsync;

  ///
  final bool Function(FlutterErrorDetails details)? onAsyncError;
  ...
  ///
  final void Function(StatefulWidget oldWidget)? didUpdateWidget;
  ...
  ///
  final Future<bool> Function()? didPopRoute;

  ///
  final Future<bool> Function(String route)? didPushRoute;

  ///
  final Future<bool> Function(RouteInformation routeInformation)?
      didPushRouteInformation;

  ///
  final void Function()? didChangeMetrics;

  ///
  final void Function()? didChangeTextScaleFactor;

  ///
  final void Function()? didChangePlatformBrightness;

  ///
  final void Function(Locale locale)? didChangeLocale;

  ///
  final void Function(AppLifecycleState state)? didChangeAppLifecycleState;

  ///
  final void Function()? inactiveLifecycleState;

  ///
  final void Function()? pausedLifecycleState;

  ///
  final void Function()? detachedLifecycleState;

  ///
  final void Function()? resumedLifecycleState;

  ///
  final void Function()? didHaveMemoryPressure;

  ///
  final void Function()? didChangeAccessibilityFeatures;

  ///
  final void Function(FlutterErrorDetails details)? onError;

  // /// The unique identifier for this State object.
  // String get identifier => _con.state!.identifier;
  //
  // /// Add a specific StateXController to this View.
  // /// Returns the StateXController's unique String identifier.
  // String add(StateXController? controller) => _con.state!.add(controller);
}

class _StateXWidgetState extends StateX<Swx> {
  /// Asynchronous operations must complete successfully.
  @override
  Future<bool> initAsync() async {
    var init = await super.initAsync();
    if (init && _widget.initAsync != null) {
      init = await _widget.initAsync!();
    }
    return init;
  }
  ...
  bool onAsyncError(FlutterErrorDetails details) {
    var on = super.onAsyncError(details);
    if (!on && _widget.onAsyncError != null) {
      on = _widget.onAsyncError!(details);
    }
    return on;
  }

  /// The framework will call this method exactly once.
  /// Only when the [Swx] object is first initiated.
  @override
  void initState() {
    _widget = widget;
    add(_widget._con);
    super.initState();
    if (_widget.initState != null) {
      _widget.initState!();
    }
  }

  // Store in a local variable and stop using the getter, widget.
  late Swx _widget;

  /// The framework calls this method whenever it removes this [Swx] object
  /// from the tree.
  @override
  void deactivate() {
    super.deactivate();
    if (_widget.deactivate != null) {
      _widget.deactivate!();
    }
  }
  ...
  void activate() {
    super.activate();
    if (_widget.activate != null) {
      _widget.activate!();
    }
  }

  /// The framework calls this when this [Swx] object will never build again.
  @override
  void dispose() {
    if (_widget.dispose != null) {
      _widget.dispose!();
    }
    super.dispose();
  }

  /// Override this method to respond when this [Swx] is recreated.
  @override
  void didUpdateWidget(StatefulWidget oldWidget) {
    super.didUpdateWidget(oldWidget);
    _widget = widget;
    if (_widget.didUpdateWidget != null) {
      _widget.didUpdateWidget!(oldWidget);
    }
  }

  /// Called when this [Swx] object is first created immediately after [initState].
  /// Otherwise called only if this [Swx] State object's Widget
  /// is a dependency of [InheritedWidget].
  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (_widget.didChangeDependencies != null) {
  ...
  Future<bool> didPopRoute() async {
    ...
    if (pop && _widget.didPopRoute != null) {
      pop = await _widget.didPopRoute!();
    }
    return pop;
  }
  ...
  @override
  Future<bool> didPushRoute(String route) async {
    var push = await super.didPushRoute(route);
    if (push && _widget.didPushRoute != null) {
      push = await _widget.didPushRoute!(route);
    }
    return push;
  }
  ...
  Future<bool> didPushRouteInformation(
      RouteInformation routeInformation) async {
    var push = await super.didPushRouteInformation(routeInformation);
    if (push && _widget.didPushRouteInformation != null) {
      push = await _widget.didPushRouteInformation!(routeInformation);
    }
    return push;
  }
  ...
  void didChangeMetrics() {
    ...
    if (_widget.didChangeMetrics != null) {
      _widget.didChangeMetrics!();
    }
  }
  ...
  void didChangeTextScaleFactor() {
    ...
    if (_widget.didChangeTextScaleFactor != null) {
      _widget.didChangeTextScaleFactor!();
    }
  }
  ...
  void didChangePlatformBrightness() {
    ...
    if (_widget.didChangePlatformBrightness != null) {
      _widget.didChangePlatformBrightness!();
    }
  }
  ...
  void didChangeLocale(Locale locale) {
    ...
    if (_widget.didChangeLocale != null) {
      _widget.didChangeLocale!(locale);
    }
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (_widget.didChangeAppLifecycleState != null) {
      _widget.didChangeAppLifecycleState!(state);
    }
  }
  ...
  void inactiveLifecycleState() {
    ...
    if (_widget.inactiveLifecycleState != null) {
      _widget.inactiveLifecycleState!();
    }
  }
  ...
  void pausedLifecycleState() {
    ...
    if (_widget.pausedLifecycleState != null) {
      _widget.pausedLifecycleState!();
    }
  }
  ...
  void detachedLifecycleState() {
    ...
    if (_widget.detachedLifecycleState != null) {
      _widget.detachedLifecycleState!();
    }
  }
  ...
  void resumedLifecycleState() {
    ...
    if (_widget.resumedLifecycleState != null) {
      _widget.resumedLifecycleState!();
    }
  }
  ...
  void didHaveMemoryPressure() {
    ...
    if (_widget.didHaveMemoryPressure != null) {
      _widget.didHaveMemoryPressure!();
    }
  }
  ...
  void didChangeAccessibilityFeatures() {
    ...
    if (_widget.didChangeAccessibilityFeatures != null) {
      _widget.didChangeAccessibilityFeatures!();
    }
  }

  /// Supply an 'error handler' routine to fire when an error occurs.
  /// Allows the user to define their own with each [Swx] object.
  @override
  // details.exception, details.stack
  void onError(FlutterErrorDetails details) {
    if (_widget.onError == null) {
      ...
      _widget.onError!(details);
    }
  }

  @override
  Widget buildWidget(BuildContext context) => _widget.builder(context);
}
